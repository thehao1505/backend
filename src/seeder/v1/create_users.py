...
from data import CONTENT_THEMES # <-- Import CONTENT_THEMES

# --- Cấu hình cho file này ---
NUM_USERS_TO_CREATE = 500
OUTPUT_FILE = "generated_data.log/users.json"

# Lấy các chủ đề duy nhất từ data.py
AVAILABLE_TOPICS = list(set(theme['topic'] for theme in CONTENT_THEMES))
AVAILABLE_TOPICS.append('general') # Thêm persona "general" (tổng quát)
...
def main():
    ...
    for i in range(NUM_USERS_TO_CREATE):
        ...
        if reg_result and 'user' in reg_result and '_id' in reg_result['user']:
            ...
            persona = random.choice(AVAILABLE_TOPICS)
